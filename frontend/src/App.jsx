      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-2 md:px-0">
          ...
          <>
            ...
            <section className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-10">
              <div className="bg-gradient-to-br from-blue-200 to-blue-50 rounded-2xl shadow-lg p-6 flex flex-col items-center hover:scale-105 transition-transform">
                ...
                <span className="text-2xl font-bold text-blue-900">₹{summary.total_spent || 0}</span>
              </div>
              <div className="bg-gradient-to-br from-green-200 to-green-50 rounded-2xl shadow-lg p-6 flex flex-col items-center hover:scale-105 transition-transform">
                <span className="text-xs text-green-700 font-semibold mb-1">Avg. Transaction</span>
                <span className="text-2xl font-bold text-green-900">₹{summary.average_transaction || 0}</span>
              </div>
            ...
            {/* Sticky Tabs */}
            <nav className="sticky top-0 z-10 bg-white/80 backdrop-blur rounded-xl shadow flex justify-center gap-2 md:gap-6 mb-8 py-2 px-2 md:px-0 transition-all">
              <button className={`px-4 py-2 rounded-lg font-semibold transition-all duration-200 ${activeTab === 'category' ? 'bg-blue-500 text-white shadow-lg scale-105' : 'text-blue-700 hover:bg-blue-100'}`} onClick={() => setActiveTab('category')}>Category</button>
              <button className={`px-4 py-2 rounded-lg font-semibold transition-all duration-200 ${activeTab === 'trend' ? 'bg-green-500 text-white shadow-lg scale-105' : 'text-green-700 hover:bg-green-100'}`} onClick={() => setActiveTab('trend')}>Monthly Trend</button>
              <button className={`px-4 py-2 rounded-lg font-semibold transition-all duration-200 ${activeTab === 'merchant' ? 'bg-yellow-500 text-white shadow-lg scale-105' : 'text-yellow-700 hover:bg-yellow-100'}`} onClick={() => setActiveTab('merchant')}>Top Merchants</button>
              <button className={`px-4 py-2 rounded-lg font-semibold transition-all duration-200 ${activeTab === 'anomaly' ? 'bg-red-500 text-white shadow-lg scale-105' : 'text-red-700 hover:bg-red-100'}`} onClick={() => setActiveTab('anomaly')}>Anomalies</button>
            </nav>
            {/* Tab Content */}
            <section className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
              ...
              <div className="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center min-h-[350px] transition-all">
                {activeTab === 'category' && (
                  <>
                    <h2 className="text-xl font-bold text-blue-700 mb-4">📈 Spending by Category</h2>
                    <Pie data={chartData} />
                  </>
                )}
                {activeTab === 'trend' && (
        ...
        {!insights && !loading && (
          <div className="flex flex-col items-center justify-center mt-20 text-gray-500 animate-fade-in">
            <img src="/vite.svg" alt="Upload" className="w-32 h-32 mb-6 opacity-80" />
